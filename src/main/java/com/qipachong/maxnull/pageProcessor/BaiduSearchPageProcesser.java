...
public class BaiduSearchPageProcesser implements PageProcessor {
    ...
    public void process(Page page) {
        //List<String> links = page.getHtml().links().regex("http://www\\.oschina\\.net/flashsword/blog/\\d+").all();
        //page.addTargetRequests(links);
        /*page.putField("id1", page.getHtml().$("#1").toString());
        page.putField("id3", page.getHtml().$("#3").toString());*/
...
//        System.out.println(page.getHtml().$("div.c-container","id").all().size());
//        System.out.println(page.getHtml().$("div.c-container h3 a","InnerHtml").all().size());
...
//        page.putField("urlX",page.getHtml().$("div.c-container h3 a","href").all());
        if (existingNumber < targetNumber) {
            pageNumber++;
            page.addTargetRequest("http://www.baidu.com/s?rn=50&wd=666&ie=UTF-8&pn=" + pageNumber*50);
        }
        Program program;
        for (int i = 0; i <urls.size() ; i++) {
            program = new Program();
